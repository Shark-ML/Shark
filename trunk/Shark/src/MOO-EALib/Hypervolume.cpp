...
struct LinearArrayIterator {
	...
	void operator++() {
		mep_currentPosition += m_noObjectives;
	}
	
	void operator++( int n ) {
		mep_currentPosition += n * m_noObjectives;
	}
	...
	T * operator[]( int n ) {
		return( mep_array + n * m_noObjectives );
	}
	
	unsigned int operator-( const LinearArrayIterator & rhs ) const {
		return( mep_currentPosition - rhs.mep_currentPosition );
	}
	
	bool operator==( const LinearArrayIterator<T> & rhs ) const {
		return( mep_currentPosition == rhs.mep_currentPosition );
	}
...
// double fonseca( double* points, double* referencePoint, unsigned int noObjectives, unsigned int noPoints ) {
// 	
// }
