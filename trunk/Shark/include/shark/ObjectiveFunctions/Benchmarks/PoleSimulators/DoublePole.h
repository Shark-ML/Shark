...
public:
  ...
  //! Number of variables
  unsigned noVars() const { if(MARKOV) return 6; return 3; }  
  ...
  void getState(RealVector &v) {
    ...
    if(MARKOV){
      ...
      v(4) = state[4] / NORMAL_POLE;
      v(5) = state[5] / NORMAL_VELO;
    }
    else {
      v(0) = state[0] / NORMAL_CART;
      v(1) = state[2] / NORMAL_POLE;
      v(2) = state[4] / NORMAL_POLE;
    }
  }
  ...
  // original jiggle function. does not return correct information. left for reproducability.
  // return "jiggle". appears to have an error? wrong terms returned.
  // should return cart pos and velo with both pole velos, but returns cart pos and velo with first pole pos and velo
  // double getJiggle() { return (fabs(state[0])+fabs(state[1])+fabs(state[2])+fabs(state[3])); }

  //! Return "jiggle", abstract representation of how much the the cart oscillates
  double getJiggle() { return (fabs(state[0])+fabs(state[1])+fabs(state[3])+fabs(state[5])); }
...
private:
  void step(double action, double *st, double *derivs)
  {
    ...
    force =  ((action - 0.5) * FORCE_MAG * 2) - (MUC*signum); 
    costheta_1 = cos(st[2]);
    sintheta_1 = sin(st[2]);
