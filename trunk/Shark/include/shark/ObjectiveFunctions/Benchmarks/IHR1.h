...
namespace shark {
	...
	*/
	struct IHR1 : 
		...
		public TraitsBoxConstraintsProvider< VectorSpace<double>::PointType, IHR1 > {
			typedef AbstractMultiObjectiveFunction< VectorSpace<double> > super;
			typedef TraitsBoxConstraintsProvider< VectorSpace<double>::PointType, IHR1 > meta;
	...
	/**
	* \brief Specializes objective function traits for the function IHR1.
	*/
	template<> 
	struct ObjectiveFunctionTraits<IHR1> {

		static IHR1::SearchPointType lowerBounds( unsigned int n ) {
			IHR1::SearchPointType result( n, -1 );
			return( result );
		}

		static IHR1::SearchPointType upperBounds( unsigned int n ) {
			IHR1::SearchPointType result( n, 1 );
			return( result );
		}
