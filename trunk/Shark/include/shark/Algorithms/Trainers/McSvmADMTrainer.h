...
public:
	...
	{
		base_type::m_name = "McSvmADMTrainer";
	}
...
class LinearMcSvmADMTrainer : public AbstractLinearSvmTrainer
{
public:
	LinearMcSvmADMTrainer(double C, double accuracy = 0.001) : AbstractLinearSvmTrainer(C, accuracy)
	{
		base_type::m_name = "LinearMcSvmADMTrainer";
	}

	void train(LinearModel<CompressedRealVector, RealVector>& model, const LabeledData<CompressedRealVector, unsigned int>& dataset)
	{
		SHARK_CHECK(! model.hasOffset(), "[LinearMcSvmADMTrainer::train] models with offset are not supported (yet).");
...
/*
		...
		PegasosType::solve(
				...
				accuracy());
*/
		QpMcLinearADM solver(dataset, dim, classes);
