...
public:
	...
	{
		base_type::m_name = "McSvmMMRTrainer";
	}
...
// 		if (svm.hasOffset()) bias = RealVectorRange(param, Range(classes * ic, classes * ic + classes));
		...
		else
		{
			...
			if (base_type::m_s2do)
				...
				solver.solveSMO(dataset.labels(), base_type::m_C, alpha, base_type::m_stoppingcondition, &prop, (svm.hasOffset() ? &bias : NULL));
		}
...
public:
	...
	{
		base_type::m_name = "LinearMcSvmMMRTrainer";
	}
