...
BOOST_AUTO_TEST_CASE( ObjectiveFunctions_RadiusMarginQuotient )
{
	...
	double radius_squared = kernel.featureDistanceSqr(dataset(0).input, dataset(3).input) / 4.0;
	double margin_squared = kernel.featureDistanceSqr(dataset(1).input, dataset(2).input) / 4.0;
