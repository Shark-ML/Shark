...
BOOST_AUTO_TEST_CASE( Models_NearestNeighbor_Regression ) {
	...
	for (int i = 0; i<6; ++i)
	{
		BOOST_CHECK_SMALL(elements(prediction)[i](0) - 4.0 * (i/2 - 1), 1e-14);
	}
}
...
BOOST_AUTO_TEST_CASE( Models_NearestNeighbor_Classification_Simple ) {
	...
	{
		BOOST_CHECK_EQUAL(elements(prediction)[i],target[i]);
		BOOST_CHECK_CLOSE(elements(soft_prediction)[i](target[i]), 2.0/3.0, 1e-12);
	}
}
...
BOOST_AUTO_TEST_CASE( Models_NearestNeighbor_Classification_Simple_Brute_Force ) {
	...
	//are classified correctly in 1-NN
	{
		...
		for (size_t i = 0; i<Points; ++i)
		{
			...
			BOOST_REQUIRE_EQUAL(target[i], elements(labels)[i]);
		}
		...
		for (size_t i = 0; i<TestPoints; ++i)
		{
			...
			BOOST_CHECK_EQUAL(bruteforceLabel, elements(testLabels)[i]);
		}
	}
