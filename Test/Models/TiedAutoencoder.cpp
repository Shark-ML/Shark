#define BOOST_TEST_MODULE MODE_TIEDAUTOENCODER
...
BOOST_AUTO_TEST_CASE( TIED_AUTOENCODER_Structure)
{
	...
	BOOST_CHECK_EQUAL(net.weightMatrix().size1(), 3u);
	BOOST_CHECK_EQUAL(net.weightMatrix().size2(), 2u);
	...
	std::size_t param = 0;
	for(std::size_t i = 0; i != net.weightMatrix().size1(); ++i){
		for(std::size_t j = 0; j != net.weightMatrix().size2(); ++j,++param){
			BOOST_CHECK_EQUAL(net.weightMatrix()(i,j), newParams(param));
		}
	}
...
BOOST_AUTO_TEST_CASE( TIED_AUTOENCODER_Value )
{
	...
	for(std::size_t i = 0; i != 100; ++i){
		...
		RealVector hidden = sigmoid(prod(net.weightMatrix(),point)+net.hiddenBias());
		RealVector output = prod(trans(net.weightMatrix()),hidden)+net.outputBias();
