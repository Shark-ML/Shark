...
//since MKLKernel is based on weighted sum kernel (which is properly tested) we don't need to do numerical testing.
//however we instantiate very function to check for compile errors and check that eval() match the hand evaluated kernels.
