...
BOOST_AUTO_TEST_CASE( CVDatasetTools_CreateSameSizeBalancedOneHot )
{
	const size_t numExamples=102;

	size_t trainSize[]={51,51};
	//number elements with label 0 per partition
	size_t zeroSize[]={26,25};
	...
	std::vector<double> validationInputs;
	for(size_t i=0;i!=2;++i){
		RegressionDataset partition = folds.training(i);
