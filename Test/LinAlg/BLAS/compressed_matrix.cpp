...
void checkCompressedMatrixStructure(std::vector<Element> const& elements, compressed_matrix<int> const& matrix){
	...
	//BOOST_REQUIRE_EQUAL(matrix. raw_storage().outer_indices_end[rows-1], matrix.raw_storage().outer_indices_begin[rows]);
	BOOST_REQUIRE_EQUAL( matrix.nnz(), elements.size());
	BOOST_REQUIRE( matrix.nnz_capacity() >= elements.size());
	BOOST_REQUIRE_EQUAL(matrix.raw_storage().outer_indices_begin[0], 0);
	...
	for(std::size_t i = 0; i != rows; ++i){
		...
		BOOST_REQUIRE(matrix.raw_storage().outer_indices_begin[i+1] >= matrix.raw_storage().outer_indices_begin[i] );//values are increasing
		BOOST_REQUIRE(matrix.raw_storage().outer_indices_end[i] >= matrix.raw_storage().outer_indices_begin[i] );//end is bigger than start
		BOOST_REQUIRE(matrix.raw_storage().outer_indices_end[i] <= matrix.raw_storage().outer_indices_begin[i+1] );//end of previous is smaller equal than start of next
		BOOST_REQUIRE_EQUAL(matrix.inner_nnz(i), rowSize );
		BOOST_REQUIRE_EQUAL(matrix.raw_storage().outer_indices_end[i] - matrix.raw_storage().outer_indices_begin[i], rowSize);
		BOOST_REQUIRE_EQUAL(matrix.raw_storage().outer_indices_begin[i+1] - matrix.raw_storage().outer_indices_begin[i], matrix.row_capacity(i));
		...
		//check row elements
		std::size_t rowIndex = matrix.raw_storage().outer_indices_begin[i];
		for(compressed_matrix<int>::const_row_iterator pos = matrix.row_begin(i); pos != matrix.row_end(i); 
			++pos,++elem,++rowIndex
		){
			//check array
			BOOST_CHECK_EQUAL(matrix.raw_storage().indices[rowIndex],elements[elem].column);
			BOOST_CHECK_EQUAL(matrix.raw_storage().values[rowIndex],elements[elem].value);
			//check iterator
...
void checkRowSizes(std::vector<std::size_t> const& rowSizes, compressed_matrix<int> const& matrix){
	...
	for(std::size_t i = 0; i != rowSizes.size(); ++i){
		BOOST_CHECK_EQUAL(matrix.raw_storage().outer_indices_begin[i], rowStart);
		BOOST_CHECK(matrix.row_capacity(i) >= rowSizes[i]);
		BOOST_CHECK(matrix.raw_storage().outer_indices_end[i] >= rowStart);
		rowStart += rowSizes[i];
	}
	//BOOST_CHECK_EQUAL(matrix.raw_storage().outer_indices_begin[rowSizes.size()], matrix.raw_storage().outer_indices_end[rowSizes.size()-1]);
