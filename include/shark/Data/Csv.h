...
void export_csv( const T & input, // Container that holds the samples
	...
	typename T::const_element_iterator iti = input.elemBegin();
	typename U::const_element_iterator itl = labels.elemBegin();


	for( ; iti != input.elemEnd(); ++iti, ++itl ) {
...
template<typename T, typename Stream>
void export_csv( const T & input, // Container that holds the samples
		 const Data<RealVector> & labels, // Container that holds the labels
		 Stream & out, // The file to be read from
		 LabelPosition lp,  // The position of the label
		 const std::string & separator, // The separator between elements
		 bool scientific = true, //scientific notation?
	...
	typename T::const_element_iterator iti = input.elemBegin();
	Data<RealVector>::const_element_iterator itl = labels.elemBegin();
...
void export_csv(
	...
	std::ofstream ofs( fn.c_str() );
	detail::export_csv( dataset.inputs(), dataset.labels(), ofs, lp, separator, sci, width );
}
