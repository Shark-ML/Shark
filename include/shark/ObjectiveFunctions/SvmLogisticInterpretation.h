...
public:
    ...
    SvmLogisticInterpretation( FoldsType const& folds, KernelType* kernel,
    ...
    : mep_kernel( kernel ),
      ...
      m_inputDims( folds.training(0).inputs()(0).size() ), //use any input point
    ...
    double eval(SearchPointType const& parameters) const
    {
        ...
        for ( unsigned int i=0; i<m_numFolds; i++ )
        {
            ...
            for ( unsigned int j=0; j<cur_vsize; j++ ) {
                tmp_helper_labels[next_label] = cur_vlabels(j);
                tmp_helper_preds[next_label] = cur_vscores(j);
                ++next_label;
            }
        }
    ...
    double evalDerivative(PType const& parameters, FirstOrderDerivative &derivative) const
    {
        ...
        for ( unsigned int i=0; i<m_numFolds; i++ )
        {
            ...
            for ( unsigned int j=0; j<cur_vsize; j++ ) {
                ...
                svm_deriv.modelCSvmParameterDerivative( cur_vinputs(j), der );
        ...
        for ( unsigned int i=0; i<m_nhp; i++ )
            derivative.m_gradient(i) = 0.0;
        double dL_dsp; //derivative of likelihood wrt sigmoid predictions
        double dsp_dsvmp; //derivative of sigmoid predictions wrt svm predictions
        double p; //current sigmoid prediction
        double ss = ( m_sigmoidSlopeIsUnconstrained ? std::exp(sigmoid_model.parameterVector()(0)) : sigmoid_model.parameterVector()(0) );
        for ( unsigned int i=0; i<m_numSamples; i++ ) {
            p = sigmoid_predictions(i)(0);
            // compute derivative of the negative log likelihood
            if ( all_validation_labels(i) == 1 ) //positive class
                dL_dsp = -1.0/p;
            else //negative class
                dL_dsp = 1.0/(1.0-p);
            // compute derivative of the sigmoid
            dsp_dsvmp = ss * p * (1.0-p); //severe sign confusion potential: p(1-p) is deriv. w.r.t. t in 1/(1+e**(-t))!
            for ( unsigned int j=0; j<m_nhp; j++ ) {
                derivative.m_gradient(j) += dL_dsp * dsp_dsvmp * all_validation_predict_derivs(i,j);
            }
        }
        for ( unsigned int i=0; i<m_nhp; i++ ) {
            // correct for the fact that AbstractLoss devides the NCLL eval scores by the number of examples when evaluated in batch mode
            derivative.m_gradient(i) /= m_numSamples;
        }
        return ncll.eval( all_validation_labels, sigmoid_predictions );
    }
};
