...
namespace shark {
	...
	*/
	struct ZDT1 : 
		...
		public TraitsBoxConstraintsProvider< VectorSpace<double>::PointType, ZDT1 > {
			typedef AbstractMultiObjectiveFunction< VectorSpace<double> > super;
			typedef TraitsBoxConstraintsProvider< VectorSpace<double>::PointType, ZDT1 > meta;
	...
	/**
	* \brief Specializes objective function traits for the function ZDT1.
	*/
	template<> 
	struct ObjectiveFunctionTraits<ZDT1> {

		static ZDT1::SearchPointType lowerBounds( unsigned int n ) {
			ZDT1::SearchPointType result( n, 0 );
			return( result );
		}

		static ZDT1::SearchPointType upperBounds( unsigned int n ) {
			ZDT1::SearchPointType result( n, 1 );
			return( result );
		}
