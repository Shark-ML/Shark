...
namespace shark{
	...
	*/
	struct IHR4 : public AbstractMultiObjectiveFunction< VectorSpace<double> >,
		public TraitsBoxConstraintsProvider< VectorSpace<double>::PointType, IHR4 > {
			typedef AbstractMultiObjectiveFunction< VectorSpace<double> > super;
			typedef TraitsBoxConstraintsProvider< VectorSpace<double>::PointType, IHR4 > meta;

			typedef super::ResultType ResultType;
			typedef super::SearchPointType SearchPointType;

			IHR4() : super( 2 ), m_a( 1000 ) {
				m_name = "IHR4";
			...
			ResultType eval( const SearchPointType & x )const {
				...
				for (unsigned i = 1; i < numberOfVariables(); i++)
					g += sqr( y( i ) ) - 10 * ::cos( 4 * M_PI * y( i ) ); //hg( y( i ) );
	...
	/**
	* \brief Specializes objective function traits for the function IHR4.
	*/
	template<> 
	struct ObjectiveFunctionTraits<IHR4> {

		static IHR4::SearchPointType lowerBounds( unsigned int n ) {
			IHR4::SearchPointType result( n, -5 );
			return( result );
		}

		static IHR4::SearchPointType upperBounds( unsigned int n ) {
			IHR4::SearchPointType result( n, 5 );
			return( result );
		}
