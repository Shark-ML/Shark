...
public:
	...
	void evalLayer(std::size_t layer,RealMatrix const& patterns,RealMatrix& outputs)const{
		...
		if(layer == 0){//input->hidden
			SIZE_CHECK(patterns.size2() == weightMatrix().size2());
			std::size_t numOutputs = weightMatrix().size1();
			outputs.resize(numPatterns,numOutputs);
			outputs.clear();
			axpy_prod(patterns,trans(weightMatrix()),outputs);
			noalias(outputs) += repeat(hiddenBias(),numPatterns);
			noalias(outputs) = m_hiddenNeuron(outputs);
		}
		else{//hidden->output
			SIZE_CHECK(patterns.size2() == weightMatrix().size1());
			std::size_t numOutputs = weightMatrix().size2();
			outputs.resize(numPatterns,numOutputs);
			outputs.clear();
			axpy_prod(patterns,weightMatrix(),outputs);
...
private:
	...
	void computeDelta(
		...
		axpy_prod(outputDelta,trans(weightMatrix()),hiddenDelta,true);
	...
	void computeDelta(
		...
		axpy_prod(hiddenDelta,weightMatrix(),inputDelta,true);
	}
