...
namespace shark {
/// \brief Finds the least/largest Contributors given 2D points
///
/// The algorithm sorts the points by their first coordinate and then 
/// calculates the volume of the hypervolume dominated by each point separately
...
private:
	...
	std::vector<std::size_t> bestContributors( std::vector<Point> const& front, std::size_t k, Comparator comp)const{
		...
		//compute the hypervalue contribution for each point except the endpoints
		*heapEnd = std::make_pair((front[2].f1 - front[1].f1) * (front[0].f2 - front[1].f2),front[1].index);
		++heapEnd;
		for(std::size_t i = 2; i < front.size()-1;++i){
		...
		for(auto const& entry: bestK){
			index.push_back(entry.second);
		}
		return index;
	}
public:
	...
	///
	/// \param [in] points The set \f$S\f$ of points from which to select the least contributor.
	...
	std::vector<std::size_t> least( Set const& points, std::size_t k, VectorType const& referencePoint)const{
	...
	std::vector<std::size_t> least( Set const& points, std::size_t k)const{
		std::vector<Point > front;
		for(std::size_t i  = 0; i != points.size(); ++i)
			front.emplace_back(points[i][0],points[i][1],i);
		std::sort(front.begin()+1,front.end()-1);
	...
	/// \param [in] referencePoint The reference point \f$\vec{r} \in \mathbb{R}^2\f$ for the hypervolume calculation, needs to fulfill: \f$ \forall s \in S: s \preceq \vec{r}\f$.
	template<typename Set, typename VectorType>
	std::vector<std::size_t> largest( Set const& points, std::size_t k, VectorType const& referencePoint)const{
		std::vector<Point > front;
		front.emplace_back(0,referencePoint[1],points.size()+1);//add reference point
		for(std::size_t i  = 0; i != points.size(); ++i)
			front.emplace_back(points[i][0],points[i][1],i);
	...
	/// \param [in] k The number of points to select.
	template<typename Set>
	std::vector<std::size_t> largest( Set const& points, std::size_t k)const{
		std::vector<Point > front;
		for(std::size_t i  = 0; i != points.size(); ++i)
			front.emplace_back(points[i][0],points[i][1],i);
		std::sort(front.begin()+1,front.end()-1);
