...
public:
	...
	void updateSMO(std::size_t v, std::size_t w){
		...
		if (v == w)
		{
			...
			if(varsum < 1.e-12 || m_C-varsum < 1.e-12){
				recomputeAlphaSum(i);
			}
		}
		else
		{
			...
			if(iv == iw){
				double upperBoundc = m_C-varsum+m_alpha(v);
				...
				double Uv = m_C-varsum+m_alpha(v);
				double Uw = m_C-varsum+m_alpha(w);
				// solve the sub-problem and update the gradient using the step sizes mu
				detail::solveQuadratic2DBox(m_alpha(v), m_alpha(w),
					m_gradient(v), m_gradient(w),
					Qvv, Qvw, Qww,
			...
			if(varsumv < 1.e-12 || m_C-varsumv < 1.e-12){
				recomputeAlphaSum(iv);
			}
			if(varsumw < 1.e-12 || m_C-varsumw < 1.e-12){
				recomputeAlphaSum(iw);
			}
...
//~ public:
	//~ typedef typename Matrix::QpFloatType QpFloatType;
	//~ BiasSolver(QpMcBoxSimplexDecomp<Matrix>* problem) : m_problem(problem){}
