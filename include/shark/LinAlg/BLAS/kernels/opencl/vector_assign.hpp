...
void apply(vector_expression<V, opencl_tag>& v_unreg, F const& f_unreg) {
...
template<class F, class V>
void assign(vector_expression<V, opencl_tag>& v, typename V::value_type t) {
	...
	auto f = device_traits<opencl_tag>::make_bind_second(F(), t);
	apply(v,f);
}
...
void vector_assign_functor(
	vector_expression<V, opencl_tag>& v_unreg,
	vector_expression<E, opencl_tag> const& e_unreg,
	F f_unreg,
	dense_tag, dense_tag
) {
	if(v_unreg().size() == 0) return;
...
void vector_assign(
	vector_expression<V, opencl_tag>& v, vector_expression<E, opencl_tag> const& e, 
	dense_tag t, dense_tag
) {
	vector_assign_functor(v, e, device_traits<opencl_tag>::right_arg<typename E::value_type>(), t, t);
}
