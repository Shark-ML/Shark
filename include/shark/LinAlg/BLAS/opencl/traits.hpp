 ...
 * 
 *
 * \brief       Traits of opencl expressions
 *
 * \author      O. Krause
 * \date        2016
 *
...
#ifndef REMORA_OPENCL_TRAITS_HPP
#define REMORA_OPENCL_TRAITS_HPP
...
template<>
struct device_traits<opencl_tag>{
	...
	struct safe_divide{
		...
		template<class Arg1, class Arg2>
		opencl::detail::invoked_safe_div<Arg1,Arg2, T,S> operator()(const Arg1 &x, const Arg2& y) const
		{
	...
	struct multiply_and_add{
		...
		template<class Arg1, class Arg2>
		opencl::detail::invoked_multiply_and_add<Arg1,Arg2,T,S> operator()(const Arg1 &x, const Arg2& y) const
		{
			return {x,y, m_scalar};
		}
		S m_scalar;
	...
	struct operator_scalar{
		...
		template<class Arg1>
		opencl::detail::invoked_operator_scalar<Arg1,T, Op, S> operator()(Arg1 const& x) const
		{
			return {x, m_scalar};
		}
		S m_scalar;
	...
	struct multiply_assign{
		...
		template<class Arg1, class Arg2>
		opencl::detail::invoked_operator_scalar<Arg2,T,'*',S> operator()(const Arg1&, const Arg2& y) const
		{
			return {y, m_scalar};
		}
		S m_scalar;
	...
	struct constant{
		...
		template<class Arg>
		opencl::detail::invoked_constant<T,S> operator()(Arg const&) const
		{
			return {m_value};
		}
		template<class Arg1, class Arg2>
		opencl::detail::invoked_constant<T,S> operator()(Arg1 const&, Arg2 const&) const
		{
			return {m_value};
		}
	...
	struct sqr{
		...
		template<class Arg1>
		opencl::detail::invoked_sqr<Arg1,T> operator()(const Arg1 &x) const{
			return {x};
		}
	};
	template<class T>
	struct soft_plus{
		...
		template<class Arg1>
		opencl::detail::invoked_soft_plus<Arg1,T> operator()(const Arg1 &x) const{
			return {x};
		}
	};
	template<class T>
	struct sigmoid{
		...
		template<class Arg1>
		opencl::detail::invoked_sigmoid<Arg1,T> operator()(const Arg1 &x) const{
			return {x};
		}
	};
	template<class T>
	struct inv{
		...
		template<class Arg1>
		opencl::detail::invoked_inv<Arg1,T> operator()(const Arg1 &x) const{
			return {x};
		}
	};
...
template<class F, class Arg2>
struct register_with_compute_kernel<device_traits<opencl_tag>::template bind_second<F,Arg2> >{
	typedef typename register_with_compute_kernel<F>::type f_type;
	typedef device_traits<opencl_tag>::template bind_second<f_type,std::string> type;
	static type reg(
		meta_kernel& k,
		device_traits<opencl_tag>::template bind_second<F,Arg2> const& f
	){
...
template<class F, class G>
struct register_with_compute_kernel<device_traits<opencl_tag>::template compose<F, G> >{
	typedef typename register_with_compute_kernel<F>::type f_type;
	typedef typename register_with_compute_kernel<G>::type g_type;
	typedef typename device_traits<opencl_tag>::template compose<f_type, g_type> type;
	static type reg(
		meta_kernel& k,
		device_traits<opencl_tag>::compose<F, G> const& composed
	){
...
template<class F1, class F2, class G>
struct register_with_compute_kernel<device_traits<opencl_tag>::template compose_binary<F1, F2, G> >{
	...
	typedef typename register_with_compute_kernel<G>::type g_type;
	typedef typename device_traits<opencl_tag>::template compose_binary<f1_type, f2_type, g_type> type;
	static type reg(
		meta_kernel& k,
		device_traits<opencl_tag>::compose_binary<F1, F2, G> const& composed
	){
...
template<class F1, class F2, class G>
struct register_with_compute_kernel<device_traits<opencl_tag>::template transform_arguments<F1, F2, G> >{
	typedef typename register_with_compute_kernel<F1>::type f1_type;
	typedef typename register_with_compute_kernel<F2>::type f2_type;
	typedef typename register_with_compute_kernel<G>::type g_type;
	typedef typename device_traits<opencl_tag>::template transform_arguments<f1_type, f2_type, g_type> type;
	static type reg(
		meta_kernel& k,
		device_traits<opencl_tag>::transform_arguments<F1, F2, G> const& composed
	){
		auto f1_reg = register_with_compute_kernel<F1>::reg(k,composed.m_f1);
		auto f2_reg = register_with_compute_kernel<F2>::reg(k,composed.m_f2);
		auto g_reg = register_with_compute_kernel<G>::reg(k,composed.m_g);
...
template<class T>
struct register_with_compute_kernel<device_traits<opencl_tag>::template constant<T,T> >{
	typedef typename device_traits<opencl_tag>::template constant<T,std::string> type;
	static type reg(
		meta_kernel& k,
		device_traits<opencl_tag>::constant<T,T> const& f
	){
...
template<class T>
struct register_with_compute_kernel<device_traits<opencl_tag>::template safe_divide<T,T> >{
	typedef typename device_traits<opencl_tag>::template safe_divide<T,std::string> type;
	static type reg(
		meta_kernel& k,
		device_traits<opencl_tag>::safe_divide<T,T> const& f
	){
...
template<class T>
struct register_with_compute_kernel<device_traits<opencl_tag>::template multiply_and_add<T,T> >{
	typedef typename device_traits<opencl_tag>::template multiply_and_add<T,std::string> type;
	static type reg(
		meta_kernel& k,
		device_traits<opencl_tag>::multiply_and_add<T,T> const& f
	){
...
struct register_with_compute_kernel<device_traits<opencl_tag>::template operator_scalar<T, Op, T> >{
	typedef typename device_traits<opencl_tag>::template operator_scalar<T, Op, std::string> type;
	static type reg(
		meta_kernel& k,
		device_traits<opencl_tag>::operator_scalar<T, Op, T> const& f
	){
		return type(k.register_kernel_arg(f.m_scalar));
	}
};
...
template<class T>
struct register_with_compute_kernel<device_traits<opencl_tag>::template multiply_assign<T,T> >{
	typedef typename device_traits<opencl_tag>::template multiply_assign<T,std::string> type;
	static type reg(
		meta_kernel& k,
		device_traits<opencl_tag>::multiply_assign<T,T> const& f
	){
		return type(k.register_kernel_arg(f.m_scalar));
	}
};
...
struct dense_vector_element{
	...
	template<class Arg>
	opencl::detail::invoked_dense_vector_element<Arg,T, S> operator()(Arg const& x) const{
...
struct dense_matrix_element{
	...
	template<class Arg1, class Arg2>
	opencl::detail::invoked_matrix_element<Arg1, Arg2, T, S> operator()(Arg1 const& x, Arg2 const& y) const{
