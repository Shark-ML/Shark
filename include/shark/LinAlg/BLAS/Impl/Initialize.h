...
private:
	Matrix& m_matrix;
	typedef typename Matrix::const_iterator1 Iterator;
	typedef typename Matrix::const_iterator2 IndexIterator;
public:
	MatrixExpression(Matrix& matrix):m_matrix(matrix){}
	template<class Iter>
	void init(Iter& pos)const{
		for(Iterator outer = m_matrix.begin1(); outer != m_matrix.end1(); ++outer){
			for(IndexIterator inner = outer.begin(); inner != outer.end(); ++inner,++pos){
				*pos = *inner;
			}
		}
	}
	...
	void split(Iter& pos){
		for(Iterator outer = m_matrix.begin1(); outer != m_matrix.end1(); ++outer){
			for(IndexIterator inner = outer.begin(); inner != outer.end(); ++inner,++pos){
				m_matrix(inner.index1(),inner.index2()) = *pos;
			}
		}
	}
	std::size_t size()const{
		std::size_t elements = 0;
		for(Iterator pos = m_matrix.begin1(); pos != m_matrix.end1(); ++pos){
			IndexIterator pos1Iter=pos.begin();
			IndexIterator pos2Iter=pos.end();
...
public:
	...
	template<class Iter>
	void init(Iter& pos)const{
		RealVector parameters = m_parameterizable.parameterVector();
		VectorExpression<RealVector&> vectorWriter(parameters);
		vectorWriter.init(pos);
	}
	template<class Iter>
	void split(Iter& pos){
		RealVector parameters(size());
		VectorExpression<RealVector&> vectorWriter(parameters);
...
///\brief Wrapper representing a range of vectors or matrices like std::vector<RealVector> or std::list<SparseRealMatrix>.
///
...
//Unfortunately, due to the heterogenity of the arguments, a lot of defines need to be used to prevent double or trippling code
