/*!
 * \brief       Implements the dense matrix class for the opencl
 * 
 * \author      O. Krause
 * \date        2016
 *
 ...
 *
 */
#ifndef REMORA_OPENCL_DENSE_HPP
#define REMORA_OPENCL_DENSE_HPP
...
class dense_vector_adaptor<T, Tag, opencl_tag>: public vector_expression<dense_vector_adaptor<T, Tag, opencl_tag>, opencl_tag > {
public:
	...
	typedef dense_vector_adaptor<T const, Tag, opencl_tag> const_closure_type;
	typedef dense_vector_adaptor closure_type;
	typedef opencl::dense_vector_storage<T, Tag> storage_type;
	typedef opencl::dense_vector_storage<value_type const, Tag> const_storage_type;
	...
	dense_vector_adaptor(dense_vector_adaptor<U, Tag2, opencl_tag> const& v)
	...
	dense_vector_adaptor(vector<value_type, opencl_tag> const& v)
	: m_storage(v().raw_storage())
	, m_queue(&v().queue())
	, m_size(v().size()){}
	
	dense_vector_adaptor(vector<value_type, opencl_tag>& v)
	: m_storage(v().raw_storage())
	, m_queue(&v().queue())
	, m_size(v().size()){}
	...
	template<class E>
	dense_vector_adaptor& operator = (vector_expression<E, opencl_tag> const& e) {
		REMORA_SIZE_CHECK(size() == e().size());
		return assign(*this, typename vector_temporary<dense_vector_adaptor>::type(e));
	}
...
private:
	...
	dense_vector_adaptor(vector<value_type, opencl_tag> && v);//no construction from temporary vector
...
class dense_matrix_adaptor<T, Orientation, Tag, opencl_tag>: public matrix_expression<dense_matrix_adaptor<T,Orientation, Tag, opencl_tag>, opencl_tag > {
public:
	typedef std::size_t size_type;
	typedef typename std::remove_const<T>::type value_type;
	typedef value_type const& const_reference;
	...
	typedef dense_matrix_adaptor<value_type const, Orientation, Tag, opencl_tag> const_closure_type;
	typedef opencl::dense_matrix_storage<T, Tag> storage_type;
	typedef opencl::dense_matrix_storage<value_type const, Tag> const_storage_type;
        typedef Orientation orientation;
	typedef elementwise<dense_tag> evaluation_category;

	// Construction and destruction
	template<class U, class Tag2>
	dense_matrix_adaptor(dense_matrix_adaptor<U, Orientation, Tag2, opencl_tag> const& expression)
	...
	dense_matrix_adaptor(matrix<value_type, Orientation, opencl_tag> const& m )
	: m_storage(m().raw_storage())
	, m_queue(&m().queue())
	, m_size1(m().size1())
	, m_size2(m().size2()){}
	
	dense_matrix_adaptor(matrix<value_type, Orientation, opencl_tag>& m )
	: m_storage(m().raw_storage())
	, m_queue(&m().queue())
	, m_size1(m().size1())
	, m_size2(m().size2()){}
	...
	template<class E>
	dense_matrix_adaptor& operator = (matrix_expression<E, opencl_tag> const& e) {
		REMORA_SIZE_CHECK(size1() == e().size1());
		REMORA_SIZE_CHECK(size2() == e().size2());
		return assign(*this, typename matrix_temporary<dense_matrix_adaptor>::type(e));
	}
	template<class E>
	dense_matrix_adaptor& operator = (vector_set_expression<E, opencl_tag> const& e) {
...
template<class T>
class vector<T, opencl_tag>: public vector_container<vector<T, opencl_tag>, opencl_tag > {
public:
	...
	typedef dense_vector_adaptor<T const, continuous_dense_tag, opencl_tag> const_closure_type;
	typedef dense_vector_adaptor<T, continuous_dense_tag, opencl_tag> closure_type;
	typedef opencl::dense_vector_storage<T,continuous_dense_tag> storage_type;
	typedef opencl::dense_vector_storage<T,continuous_dense_tag> const_storage_type;
	typedef elementwise<dense_tag> evaluation_category;
	...
	template<class E>
	vector(vector_expression<E, opencl_tag> const& e)
	...
	template<class E>
	vector(vector_expression<E, opencl_tag> const& e, boost::compute::command_queue& queue)
	...
	vector& operator = (vector_container<C, opencl_tag> const& v) {
		resize(v().size());
		return assign(*this, v);
	}
	...
	template<class E>
	vector& operator = (vector_expression<E, opencl_tag> const& e) {
...
/// \brief A dense matrix of values of type \c T stored on the opencl
///
...
class matrix<T,L, opencl_tag> : public matrix_container<matrix<T,L, opencl_tag>, opencl_tag > {
public:
	typedef T value_type;
	typedef value_type const_reference;
	typedef value_type reference;
	...
	typedef dense_matrix_adaptor<T const,L, continuous_dense_tag, opencl_tag> const_closure_type;
	typedef dense_matrix_adaptor<T,L, continuous_dense_tag, opencl_tag> closure_type;
	typedef opencl::dense_matrix_storage<T, continuous_dense_tag> storage_type;
	typedef opencl::dense_matrix_storage<T const, continuous_dense_tag> const_storage_type;
	typedef elementwise<dense_tag> evaluation_category;
	...
	template<class E>
	matrix(matrix_expression<E, opencl_tag> const& e)
	...
	template<class E>
	matrix(vector_set_expression<E, opencl_tag> const& e)
	...
	/// \return a reference to a matrix (i.e. the destination matrix)
	template<class C>          // Container assignment without temporary
	matrix& operator = (matrix_container<C, opencl_tag> const& m) {
		resize(m().size1(), m().size2());
		return assign(*this, m);
	}
	...
	template<class E>
	matrix& operator = (matrix_expression<E, opencl_tag> const& e) {
		matrix temporary(e);
		swap(*this,temporary);
		return *this;
	}
	...
	/// \return a reference to the resulting matrix
	template<class E>
	matrix& operator = (vector_set_expression<E, opencl_tag> const& e) {
		matrix temporary(e);
		swap(temporary);
		return *this;
	}
...
class dense_triangular_proxy<T, Orientation, triangular_tag<Upper, Unit> , opencl_tag>
: public matrix_expression<dense_triangular_proxy<T, Orientation, triangular_tag<Upper, Unit>, opencl_tag>, opencl_tag> {
public:
	typedef std::size_t size_type;
	typedef typename std::remove_const<T>::type value_type;
	...
	typedef value_type const& const_reference;
	typedef dense_triangular_proxy<value_type const, Orientation, triangular_tag<Upper, Unit> , opencl_tag> const_closure_type;
	typedef dense_triangular_proxy<T, Orientation, triangular_tag<Upper, Unit> , opencl_tag> closure_type;

	typedef opencl::dense_matrix_storage<T, dense_tag> storage_type;
	typedef opencl::dense_matrix_storage<value_type const, dense_tag> const_storage_type;
	...
	template<class U>
	dense_triangular_proxy(dense_triangular_proxy<U, Orientation, triangular_tag<Upper, Unit>, opencl_tag> const& expression)
	: m_storage(expression.raw_storage())
	, m_queue(&expression.queue())
	, m_size1(expression.size1())
	, m_size2(expression.size2()){}
	...
	dense_matrix_adaptor<T, Orientation, dense_tag, opencl_tag> to_dense() const{
...
struct vector_to_matrix_optimizer<dense_vector_adaptor<T, continuous_dense_tag, opencl_tag>, Orientation >{
	typedef dense_matrix_adaptor<T, Orientation, continuous_dense_tag, opencl_tag> type;
	
	static type create(
		dense_vector_adaptor<T, continuous_dense_tag, opencl_tag> const& v,
		std::size_t size1, std::size_t size2
	){
		opencl::dense_matrix_storage<T, continuous_dense_tag> storage = {v.raw_storage().buffer, v.raw_storage().offset, Orientation::index_m(size1,size2)};
