 ...
 *
 */
#ifndef REMORA_KERNELS_OPENCL_POTRF_HPP
#define REMORA_KERNELS_OPENCL_POTRF_HPP
...
potrf_kernel createPotrfDiagBlockKernel(
	matrix_expression<MatA, opencl_tag>& A_unreg,
	char const* options
){
...
void potrf_recursive(
	matrix_expression<MatA, opencl_tag>& Afull,
	std::size_t start,
	std::size_t end,
	potrf_kernel& kernel
){
...
template <typename MatA>
void potrf_dispatch(
	matrix_expression<MatA, opencl_tag>& A,
	lower
){
...
std::size_t potrf(
	matrix_container<MatA, opencl_tag>& A
){
